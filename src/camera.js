...
export function createCamera(gameWindow) {
    ...
    const ZOOM_SENSIVITY = 0.02;
    const PAN_SENSIVITY = -0.01;
    ...
    function onMouseMove(event) {
        ...
        if (isMiddleMouseDown) {
            ...
            cameraOrigin.add(forward.multiplyScalar(PAN_SENSIVITY * deltaY));
            cameraOrigin.add(left.multiplyScalar(PAN_SENSIVITY * deltaX));
            updateCameraPosition();
        }

        if (isRightMouseDown) {
            cameraRadius += deltaY * ZOOM_SENSIVITY;
